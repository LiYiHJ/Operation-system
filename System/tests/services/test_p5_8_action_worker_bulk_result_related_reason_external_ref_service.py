...
def test_action_queue_round29_bulk_result_related_reason_external_ref_surface():
    reset_action_store()
    ...
    _, job_a = _create_and_push(entry, approval, delivery, batch_ref="batch-r29", suffix="a")
    _, job_b = _create_and_push(entry, approval, delivery, batch_ref="batch-r29", suffix="b")
    _, job_c = _create_and_push(entry, approval, delivery, batch_ref="batch-r29b", suffix="c")

    parent = queue.execute_bulk_command(
        command="release-lease",
        job_ids=[job_a, job_b, job_c],
        operator="ops-r29",
        reason="bulk_release_partial_r29",
        external_ref="ext-parent-r29",
    )
    queue.reexecute_bulk_command(
        ...
        command="dead-letter",
        operator="ops-r29",
        reason="rerun_failed_dead_letter_r29",
        external_ref="ext-rerun-r29",
    )
    lineage = queue.reexecute_bulk_command_lineage(
        ...
        operator="ops-r29",
        reason="lineage_redrive_r29",
        external_ref="ext-lineage-r29",
    )

    related = queue.get_bulk_command_related(parent["bulkCommandId"], limit=10)
    assert related["summary"]["totalResults"] == 3
    assert related["reasonSummary"]["bulk_release_partial_r29"] == 1
    assert related["reasonSummary"]["rerun_failed_dead_letter_r29"] == 1
    assert related["reasonSummary"]["lineage_redrive_r29"] == 1
    assert related["externalRefSummary"]["ext-parent-r29"] == 1
    assert related["externalRefSummary"]["ext-rerun-r29"] == 1
    assert related["externalRefSummary"]["ext-lineage-r29"] == 1

    latest = related["items"][0]
    assert latest["bulkCommandId"] == lineage["bulkCommandId"]
    assert latest["reason"] == "lineage_redrive_r29"
    assert latest["externalRef"] == "ext-lineage-r29"
