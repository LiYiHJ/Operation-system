...
def test_economics_core_rebind_prefers_config_resolve_and_keeps_fallback(tmp_path, monkeypatch):
    ...
    upload_info = runtime.run_upload(
        ...
        trace_id='trc_test_p4_round3',
        idempotency_key='idem_test_p4_round3',
    ...
    runtime.confirm_batch(
        ...
        notes='p4 round3',
        trace_id='trc_test_p4_round3_confirm',
        idempotency_key='idem_test_p4_round3_confirm',
        manual_overrides=[],
    )
    ...
    config_service.upsert_sku_cost_card({
        ...
        'overrideReason': 'pytest_seed_round3',
    })
    ...
    assert core['contractVersion'] == 'p4.economics_core_rebind.v1'
    assert core['coreSummary']['configBoundRowCount'] == 1
    assert core['coreSummary']['fallbackRowCount'] == 1
    assert core['costCoverageSummary']['configBoundRowCount'] == 1
    assert core['costCoverageSummary']['fallbackImportRowCount'] == 1

    first_item = core['items'][0]
    assert first_item['canonicalSku'] == 'SKU-001'
    assert first_item['resolvedProfileCode'] == 'default_profit_v1'
    assert first_item['coreBindingMode'] == 'config_resolve'
    assert first_item['resolvedVariableCostAmount'] == 28.0
    assert first_item['fallbackVariableCostAmount'] == 20.0
    assert first_item['variableCostAmount'] == 28.0
    assert first_item['resolvedComponentTotals']['ads_cost'] == 6.6
    assert first_item['coreCalculationState'] == 'calculated_config_bound_partial_costs'

    second_item = core['items'][1]
    assert second_item['canonicalSku'] == 'batch-1-row-2'
    assert second_item['coreBindingMode'] == 'import_partial_fallback'
    assert second_item['resolvedProfileCode'] == 'default_profit_v1'
    assert second_item['resolvedVariableCostAmount'] == 18.0
    assert second_item['fallbackVariableCostAmount'] == 18.0
    assert second_item['variableCostAmount'] == 18.0
    assert second_item['coreCalculationState'] == 'calculated_import_fallback_partial_costs'

    contract = intake_service.get_batch_economics_core_contract(batch_ref)
    assert contract is not None
    assert contract['consumerContract']['contractVersion'] == 'p4.economics_core_rebind.v1'
    assert 'coreBindingMode' in contract['consumerContract']['configBindingFields']
    assert contract['sourceConfigContract']['consumerContract']['contractName'] == 'economics_config_resolve'
