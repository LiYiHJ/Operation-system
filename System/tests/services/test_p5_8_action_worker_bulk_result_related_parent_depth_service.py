...
def test_action_queue_round32_bulk_result_related_parent_depth_surface():
    reset_action_store()
    ...
    _, job_a = _create_and_push(entry, approval, delivery, batch_ref="batch-r32", suffix="a")
    _, job_b = _create_and_push(entry, approval, delivery, batch_ref="batch-r32", suffix="b")
    _, job_c = _create_and_push(entry, approval, delivery, batch_ref="batch-r32b", suffix="c")

    parent = queue.execute_bulk_command(
        command="release-lease",
        job_ids=[job_a, job_b, job_c],
        operator="ops-r32",
        reason="bulk_release_partial_r32",
    )
    queue.reexecute_bulk_command(
        ...
        command="dead-letter",
        operator="ops-r32",
        reason="rerun_failed_as_dead_letter_r32",
    )
    lineage = queue.reexecute_bulk_command_lineage(
        ...
        operator="ops-r32",
        reason="lineage_redrive_r32",
    )

    related = queue.get_bulk_command_related(parent["bulkCommandId"], limit=10)
    assert related["summary"]["totalResults"] == 3
    assert related["parentBulkCommandSummary"][parent["bulkCommandId"]] == 2
    assert related["lineageDepthSummary"]["0"] == 1
    assert related["lineageDepthSummary"]["1"] == 2

    latest = related["items"][0]
    assert latest["bulkCommandId"] == lineage["bulkCommandId"]
    assert latest["parentBulkCommandId"] == parent["bulkCommandId"]
    assert latest["lineageDepth"] == 1
