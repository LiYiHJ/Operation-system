...
def test_action_queue_round28_bulk_result_related_root_source_surface():
    reset_action_store()
    ...
    _, job_a = _create_and_push(entry, approval, delivery, batch_ref="batch-r28", suffix="a")
    _, job_b = _create_and_push(entry, approval, delivery, batch_ref="batch-r28", suffix="b")
    _, job_c = _create_and_push(entry, approval, delivery, batch_ref="batch-r28b", suffix="c")

    parent = queue.execute_bulk_command(
        command="release-lease",
        job_ids=[job_a, job_b, job_c],
        operator="ops-r28",
        reason="bulk_release_partial_r28",
    )
    queue.reexecute_bulk_command(
        ...
        command="dead-letter",
        operator="ops-r28",
        reason="rerun_failed_as_dead_letter_r28",
    )
    lineage = queue.reexecute_bulk_command_lineage(
        ...
        operator="ops-r28",
        reason="lineage_redrive_r28",
    )

    related = queue.get_bulk_command_related(parent["bulkCommandId"], limit=10)
    assert related["summary"]["totalResults"] == 3
    assert related["rootBulkCommandSummary"][parent["bulkCommandId"]] == 3
    assert related["sourceBulkCommandSummary"][parent["bulkCommandId"]] == 1

    latest = related["items"][0]
    assert latest["bulkCommandId"] == lineage["bulkCommandId"]
    assert latest["rootBulkCommandId"] == parent["bulkCommandId"]
    assert latest["sourceBulkCommandIds"] == [parent["bulkCommandId"]]
