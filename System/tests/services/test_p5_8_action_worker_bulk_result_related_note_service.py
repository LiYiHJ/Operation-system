...
def test_action_queue_round30_bulk_result_related_note_surface():
    reset_action_store()
    ...
    _, job_a = _create_and_push(entry, approval, delivery, batch_ref="batch-r30", suffix="a")
    _, job_b = _create_and_push(entry, approval, delivery, batch_ref="batch-r30", suffix="b")
    _, job_c = _create_and_push(entry, approval, delivery, batch_ref="batch-r30b", suffix="c")

    parent = queue.execute_bulk_command(
        command="release-lease",
        job_ids=[job_a, job_b, job_c],
        operator="ops-r30",
        reason="bulk_release_partial_r30",
        note="parent-note-r30",
    )
    queue.reexecute_bulk_command(
        ...
        command="dead-letter",
        operator="ops-r30",
        reason="rerun_failed_dead_letter_r30",
        note="rerun-note-r30",
    )
    lineage = queue.reexecute_bulk_command_lineage(
        ...
        operator="ops-r30",
        reason="lineage_redrive_r30",
        note="lineage-note-r30",
    )

    related = queue.get_bulk_command_related(parent["bulkCommandId"], limit=10)
    assert related["summary"]["totalResults"] == 3
    assert related["noteSummary"]["parent-note-r30"] == 1
    assert related["noteSummary"]["rerun-note-r30"] == 1
    assert related["noteSummary"]["lineage-note-r30"] == 1

    latest = related["items"][0]
    assert latest["bulkCommandId"] == lineage["bulkCommandId"]
    assert latest["note"] == "lineage-note-r30"
