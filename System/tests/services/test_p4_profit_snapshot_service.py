...
def _reset_sqlite(monkeypatch, tmp_path):
    ...
    db_path = tmp_path / 'economics_profit_snapshot.db'
...
def test_profit_snapshot_save_and_list_skeleton(tmp_path, monkeypatch):
    ...
    upload_info = runtime.run_upload(
        ...
        trace_id='trc_test_p4_round6',
        idempotency_key='idem_test_p4_round6',
    ...
    runtime.confirm_batch(
        ...
        notes='p4 round6',
        trace_id='trc_test_p4_round6_confirm',
        idempotency_key='idem_test_p4_round6_confirm',
        manual_overrides=[],
    )
    ...
    config_service.upsert_sku_cost_card({
        ...
        'overrideReason': 'pytest_seed_round6',
    })
    ...
    saved = snapshot_service.save_batch_profit_snapshot(
        ...
        operator='pytest',
        note='round6 save/list skeleton',
        filters={'strategyMode': 'balanced_profit', 'constraints': {'minMargin': 0.08}},
    )
    assert saved is not None
    assert saved['contractVersion'] == 'p4.profit_snapshot.v1'
    assert saved['source'] == 'pricing_recommend'
    assert saved['itemCount'] == 2
    assert saved['summary']['skuCount'] == 2
    assert saved['summary']['currency'] == 'CNY'

    listed = snapshot_service.list_batch_profit_snapshots(batch_ref, limit=10)
    assert listed is not None
    assert listed['contractVersion'] == 'p4.profit_snapshot.v1'
    assert listed['pagination']['returned'] == 1
    assert listed['items'][0]['snapshotId'] == saved['snapshotId']
    assert listed['items'][0]['source'] == 'pricing_recommend'
    assert listed['items'][0]['summary']['skuCount'] == 2
