...
def test_profit_solve_skeleton_uses_resolve_then_fallback(tmp_path, monkeypatch):
    ...
    upload_info = runtime.run_upload(
        ...
        trace_id='trc_test_p4_round4',
        idempotency_key='idem_test_p4_round4',
    ...
    runtime.confirm_batch(
        ...
        notes='p4 round4',
        trace_id='trc_test_p4_round4_confirm',
        idempotency_key='idem_test_p4_round4_confirm',
        manual_overrides=[],
    )
    ...
    config_service.upsert_sku_cost_card({
        ...
        'overrideReason': 'pytest_seed_round4',
    })

    intake_service = EconomicsIntakeService(root_dir, object_service=object_service, config_service=config_service)
    solved = intake_service.get_batch_profit_solve(batch_ref, limit=10, offset=0, view='all')
    assert solved is not None
    assert solved['contractVersion'] == 'p4.profit_solve.v1'
    assert solved['solveSummary']['configBoundRowCount'] == 1
    assert solved['solveSummary']['fallbackRowCount'] == 1

    first_item = solved['items'][0]
    assert first_item['canonicalSku'] == 'SKU-001'
    assert first_item['solveSourceMode'] == 'config_resolve'
    # 当前 GET /batches/{batch_ref}/solve 冻结为 batch historical aggregated solve 读面，
    # 收入基数沿用上游 fact/core 的聚合行收入（本样本为 199.5），
    # 因此本轮配置行利润骨架应为 178.1 / 171.5，而不是场景求解语义下的 78.1 / 71.5。
    assert first_item['baseContributionProfit'] == 178.1
    assert first_item['profitAfterAds'] == 171.5
    assert first_item['riskAdjustedProfit'] == 171.5
    assert first_item['dominantCostComponent'] == 'platform_fee'
    assert first_item['solveState'] == 'calculated_config_bound_profit_skeleton'

    second_item = solved['items'][1]
    assert second_item['canonicalSku'] == 'batch-1-row-2'
    assert second_item['solveSourceMode'] == 'import_partial_fallback'
    assert second_item['baseContributionProfit'] == 70.0
    assert second_item['profitAfterAds'] == 70.0
    assert second_item['riskAdjustedProfit'] == 70.0
    assert second_item['solveState'] == 'calculated_import_fallback_profit_skeleton'
    assert second_item['dominantRiskDriver'] == 'cost_config_pending'

    contract = intake_service.get_batch_profit_solve_contract(batch_ref)
    assert contract is not None
    assert contract['consumerContract']['contractVersion'] == 'p4.profit_solve.v1'
    assert 'profitAfterAds' in contract['consumerContract']['layerFields']
