...
def test_action_delivery_detail_and_history_contract():
    ...
    detail_resp = client.get(f'/api/v1/actions/requests/{request_id}/delivery')
    assert detail_resp.status_code == 200
    detail_payload = detail_resp.get_json()
    assert detail_payload['data']['requestId'] == request_id
    assert detail_payload['data']['latestDelivery']['deliveryStatus'] == 'accepted'

    history_resp = client.get(f'/api/v1/actions/requests/{request_id}/delivery-history')
    ...
    assert history_payload['data']['items'][0]['deliveryStatus'] == 'accepted'
