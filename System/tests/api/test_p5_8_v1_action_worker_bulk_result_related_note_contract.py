...
def test_action_job_round30_worker_bulk_result_related_note_contract():
    client = _build_client()
    _, job_a = _create_and_push_request(client, batch_ref='batch-r30-api', suffix='a')
    _, job_b = _create_and_push_request(client, batch_ref='batch-r30-api', suffix='b')
    _, job_c = _create_and_push_request(client, batch_ref='batch-r30b-api', suffix='c')

    parent_resp = client.post('/api/v1/actions/worker/bulk-command', json={
        'command': 'release-lease',
        'jobIds': [job_a, job_b, job_c],
        'operator': 'ops-r30-api',
        'reason': 'bulk_release_partial_r30_api',
        'note': 'parent-note-r30-api',
    })
    ...
    rerun_resp = client.post(f"/api/v1/actions/worker/bulk-results/{parent['bulkCommandId']}/re-execute", json={
        ...
        'command': 'dead-letter',
        'operator': 'ops-r30-api',
        'reason': 'rerun_failed_dead_letter_r30_api',
        'note': 'rerun-note-r30-api',
    })
    ...
    lineage_resp = client.post(f"/api/v1/actions/worker/bulk-results/{parent['bulkCommandId']}/lineage-command", json={
        ...
        'operator': 'ops-r30-api',
        'reason': 'lineage_redrive_r30_api',
        'note': 'lineage-note-r30-api',
    })
    ...
    assert payload['noteSummary']['parent-note-r30-api'] == 1
    assert payload['noteSummary']['rerun-note-r30-api'] == 1
    assert payload['noteSummary']['lineage-note-r30-api'] == 1
    assert payload['items'][0]['bulkCommandId'] == lineage['bulkCommandId']
    assert payload['items'][0]['note'] == 'lineage-note-r30-api'
