...
def test_action_handoff_command_contract_happy_path():
    ...
    resp = client.post(f'/api/v1/actions/requests/{request_id}/handoff-command', json={'command': 'route_next', 'operator': 'evan'})
    assert resp.status_code == 202
    data = resp.get_json()['data']
    assert data['requestId'] == request_id
    assert data['accepted'] is True
    assert data['handoffTarget'] == 'approver'
