...
def replace_nth(text: str, old: str, new: str, n: int, label: str) -> str:
    ...
    pos = -1
    for _ in range(n):
        pos = text.find(old, start + 1)
        if pos == -1:
            raise RuntimeError(f"[{label}] could not find occurrence #{n}")
        start = pos
...
def patch_types() -> None:
    ...
    backup = TYPES_FILE.with_suffix(".ts.bak_phase1_frontend_sync_v3")
    ...
    if count == 0:
        if "importabilityStatus?" in text and "importabilityReasons?" in text:
            print("types already contain importability fields; skipping types patch")
            return
        raise RuntimeError(f"[types] expected OLD_FIELDS occurrences, got 0")
    if count < 2:
        raise RuntimeError(f"[types] expected at least 2 OLD_FIELDS occurrences, got {count}")

    text = replace_nth(text, OLD_FIELDS, NEW_FIELDS, 1, "ImportResult fields")
    text = replace_nth(text, OLD_FIELDS, NEW_FIELDS, 2, "ConfirmImportResponse fields")
    TYPES_FILE.write_text(text, encoding="utf-8")


def patch_page() -> None:
    text = PAGE_FILE.read_text(encoding="utf-8")
    backup = PAGE_FILE.with_suffix(".tsx.bak_phase1_frontend_sync_v3")
    if not backup.exists():
        backup.write_text(text, encoding="utf-8")
...
def main() -> None:
    ...
    patch_types()
    patch_page()
    print("Applied Phase 1 frontend sync patch v3")
