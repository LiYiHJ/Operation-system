...
def replace_regex_once(text: str, pattern: re.Pattern[str], replacement: str, label: str) -> str:
...
def main() -> None:
    ...
    backup = IMPORT_SERVICE.with_suffix(".py.bak_entity_key_probe_fix_v3")
    if not backup.exists():
        backup.write_text(text, encoding="utf-8")

    text = replace_regex_once(text, TOKEN_BLOCK_PATTERN, NEW_TOKEN_BLOCK, "replace token patterns")

    if "def _collect_entity_key_probe_values(" not in text:
        anchor = "    def _build_entity_key_suggestion(\n"
        idx = text.find(anchor)
        if idx == -1:
            raise RuntimeError("[insert probe helper] could not find _build_entity_key_suggestion anchor")
        text = text[:idx] + PROBE_HELPER + text[idx:]

    text = replace_regex_once(text, BUILD_FUNC_PATTERN, NEW_BUILD_FUNC, "replace build_entity_key_suggestion")
    text = replace_regex_once(text, CALL_PATTERN, NEW_CALL, "replace helper call")

    IMPORT_SERVICE.write_text(text, encoding="utf-8")
    print("Applied entity key probe fix v3")
