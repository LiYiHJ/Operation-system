...
OLD_SUGGESTION_BLOCK = """    ENTITY_KEY_TOKEN_PATTERNS = [
...
NEW_SUGGESTION_BLOCK = """    ENTITY_KEY_TOKEN_PATTERNS = [
...
def main() -> None:
    ...
    backup = IMPORT_SERVICE.with_suffix(".py.bak_entity_key_probe_fix_v1")
    if not backup.exists():
        backup.write_text(text, encoding="utf-8")

    text = replace_once(text, OLD_SUGGESTION_BLOCK, NEW_SUGGESTION_BLOCK, "replace suggestion block")
    text = replace_once(text, OLD_CALL, NEW_CALL, "pass df to entity suggestion helper")

    IMPORT_SERVICE.write_text(text, encoding="utf-8")
    print("Applied entity key probe fix v1")
