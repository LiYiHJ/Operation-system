...
def patch_types() -> None:
    ...
    backup = TYPES_FILE.with_suffix(".ts.bak_phase1_frontend_sync")
...
def patch_page() -> None:
    ...
    backup = PAGE_FILE.with_suffix(".tsx.bak_phase1_frontend_sync")
    if not backup.exists():
        backup.write_text(text, encoding="utf-8")
...
def main() -> None:
    ...
    patch_types()
    patch_page()
    print("Applied Phase 1 frontend sync patch")
