...
OPERATOR = "import_gate_regression_v2"
SHOP_ID = 1
...
def run_curl(args: list[str]) -> str:
    ...
    if proc.returncode != 0:
        raise RuntimeError(
            f"curl failed: {' '.join(args)}\nSTDOUT:\n{proc.stdout}\nSTDERR:\n{proc.stderr}"
        )
    return proc.stdout.strip()


def post_upload(sample_path: Path) -> dict[str, Any]:
    out = run_curl([
        "curl.exe", "-sS", "-X", "POST",
...
def post_confirm(session_id: int) -> dict[str, Any]:
...
def main() -> None:
    ...
    out_file = OUT_DIR / f"p0_import_gate_regression_v2_{timestamp}.json"

    rows: list[dict[str, Any]] = []

    for sample_name, sample_path in SAMPLES:
        item: dict[str, Any] = {"sample": sample_name, "path": str(sample_path)}
        ...
        item["upload"] = {
            ...
            "importabilityStatus": upload.get("importabilityStatus"),
            "importabilityReasons": upload.get("importabilityReasons"),
        ...
        item["confirm"] = None if not confirm else {
            ...
            "stagingRows": confirm.get("stagingRows"),
            ...
            "runtimeAudit": confirm.get("runtimeAudit"),
        }
