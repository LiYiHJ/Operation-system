...
class IntelligentFieldMapperV2:
    """智能字段映射器 V2 - 修复版"""

    def __init__(self):
        self.field_patterns = self._init_field_patterns()
        self.fuzzy_threshold = 0.6  # ✅ 降低置信度阈值：0.7 → 0.6
    ...
    def _init_field_patterns(self) -> Dict[str, FieldPattern]:
        ...
        return {
            ...
            "sku": FieldPattern(
                ...
                priority=100,
                validator=lambda x: bool(re.match(r'^[A-Za-z0-9\-_]+$', str(x)))
            ),
    ...
    def detect_field(self, column_name: str, sample_data: Optional[pd.Series] = None) -> Tuple[Optional[str], float, List[str]]:
        ...
        for field_name, pattern in self.field_patterns.items():
            ...
            if sample_data is not None and pattern.validator:
                ...
                if validation_score > 0:
                    score += validation_score * 0.3  # ✅ 调整权重
                    reasons.append(f"数据验证通过({validation_score:.2f})")

            # 2.4 优先级加成（0-0.5 分）
            priority_bonus = pattern.priority / 200  # ✅ 修复：100 → 200
        ...
        # 4. 计算置信度（✅ 修复：新的归一化公式）
        # 任何匹配就给基础分 0.3，然后根据匹配质量加分（最多 0.7）
        base_score = 0.3  # ✅ 降低基础分：0.5 → 0.3
        quality_score = min(best_match[1] * 0.7, 0.7)  # ✅ 提高质量分上限：0.5 → 0.7
        confidence = base_score + quality_score  # 范围：0.3-1.0
    ...
    def auto_map_columns(self, df: pd.DataFrame) -> Tuple[pd.DataFrame, Dict[str, Tuple[str, float, List[str]]]]:
        ...
        for col in df.columns:
            ...
            if standard_name:
                ...
                # 打印映射结果
                status = "✅" if confidence >= 0.6 else "⚠️" if confidence >= 0.4 else "❓"  # ✅ 降低阈值
...
if __name__ == "__main__":
    ...
    # 智能映射 V2
    mapper = IntelligentFieldMapperV2()
