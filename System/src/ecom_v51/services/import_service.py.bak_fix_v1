...
class ImportService:
    ...
    def __init__(self) -> None:
        ...
        self.dataset_registry = DatasetRegistryService(self._root_dir)
    ...
    def _resolve_dataset_contract(
        ...
        return self.dataset_registry.get_dataset(
            dataset_kind=dataset_kind,
            import_profile=import_profile,
        )

    def _apply_dataset_contract(
        self,
        ...
        *,
        ...
        contract = self._resolve_dataset_contract(
            dataset_kind=dataset_kind or payload.get("datasetKind"),
            import_profile=import_profile or payload.get("importProfile"),
        )
        payload["datasetKind"] = contract.get("datasetKind") or payload.get("datasetKind") or "orders"
        payload["importProfile"] = contract.get("importProfile") or payload.get("importProfile") or payload["datasetKind"]
        payload.setdefault("sourceType", contract.get("sourceType") or "file")
        payload.setdefault("platform", contract.get("platform") or payload.get("platform") or "generic")
        payload.setdefault("grain", contract.get("grain") or "")
        payload.setdefault("loaderTarget", contract.get("loaderTarget") or "")
        payload.setdefault("gatePolicy", contract.get("gatePolicy") or "core_safe")
        payload.setdefault("schemaVersion", contract.get("schemaVersion") or "v1")
        payload.setdefault("requiredCoreFields", list(contract.get("requiredCoreFields") or []))
        payload.setdefault("optionalCommonFields", list(contract.get("optionalCommonFields") or []))
        payload.setdefault("entityKeyField", contract.get("entityKeyField") or "sku")
        return payload
    ...
    def parse_import_file(
        ...
        if error or default_df is None:
            return self._apply_dataset_contract({
            ...
            }, dataset_kind=dataset_contract.get("datasetKind"), import_profile=dataset_contract.get("importProfile"))
        ...
        result = self._apply_dataset_contract(
            result,
    ...
    def confirm_import(
        ...
        if session_id not in self._sessions:
            return self._apply_dataset_contract({
                ...
                "semanticAcceptanceReason": [],
                "recoverySummary": {},
            }, dataset_kind=dataset_kind, import_profile=import_profile)
